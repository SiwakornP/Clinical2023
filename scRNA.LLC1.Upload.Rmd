...
LLCDF$DE.LLC.Celltype3.df = df
#LLCDF$DE.Tcell.Identity.Manual.df =df
```
...
Idents(LLC) = LLC$Identity.Celltype2
for(i in 1){
      PDF("Ext.Fig.4A.LLC.DM", w=6,h=6)
      print(DimPlot(LLC,label =F, pt.size = 0.1,
                    cols = LLCDF$Col.Identity.Celltype2
                    ) + NoLegend())
      dev.off()
      PDF("Ext.Fig.4A.LLC.DM.L", w=9,h=6)
      print(DimPlot(LLC,label =T, 
                    cols = LLCDF$Col.Identity.Celltype2
                    ) ) 
      dev.off()
...
#GSEA
```{r}
GSEA = readRDS( "~/Library/CloudStorage/OneDrive-Personal/Documents_Onedrive/RStudioProject/LLC/GSEA.LLC.Celltype3.230720.rds")
selected.pathway = c("GERY_CEBP_TARGETS","HALMOS_CEBPA_TARGETS_UP","TAVOR_CEBPA_TARGETS_UP",
                     "HALLMARK_INTERFERON_GAMMA_RESPONSE","HALLMARK_OXIDATIVE_PHOSPHORYLATION","HALLMARK_IL2_STAT5_SIGNALING")
selected.pathway = c("HALLMARK_IL2_STAT5_SIGNALING",
...
for(t in names(DE)){
    for(i in names(DE[[t]])){
    df2 = GSEA[[t]][[i]] %>% as.data.frame() %>% filter(pathway %in% selected.pathway)
    df2$Dataset = t
    df2$Cluster = i
    df = rbind(df,df2)
  }
...
df$Cluster = factor(df$Cluster, levels = names(DE[[t]]))
df$padj_cat = "> 0.05"
df$padj_cat[c(df$padj<0.05)] = "<0.05"
plot1 = df |> ggplot(aes(y = fct_rev(Cluster))) + 
  geom_point(shape = 18,size =3,aes(x = NES)) +
  geom_vline(xintercept = 0 ,linetype="dotdash", )+
  #xlim(-2.5,2.5)+
  theme_classic() +
  facet_grid(Dataset~pathway,switch = "y") +
  ylab("Dataset")+
  theme(axis.text = element_text(color = "black"),
        panel.border = element_rect(color = "black", fill = NA, size =1),
        strip.text.x = element_text(face = "bold",size = 7),
        strip.text.y.left = element_text(face = "bold", size = 8),
        strip.placement = "outside",
        axis.title.y = element_blank())
for(i in 1){
  PNG("LLC.GSEA.Overview.23071",w=15,h=6,r=200)
  print(plot1)
  dev.off()
}
...
for(t in names(DE)){
  for(i in names(DE[[t]])){
  #for(i in c("T_CD8","T_CD4","Treg")){
    df1 = DE[[t]][[i]] %>% as_tibble(rownames = "Genes") %>% dplyr::select(Genes, avg_log2FC) %>% arrange(avg_log2FC) 
    ranks<- deframe(df1)
    df2 = GSEA[[t]][[i]] %>% as.data.frame()
    rownames(df2) = df2$pathway
    for(p in selected.pathway){
      tmp3 = paste0("NES = ",df2[p,"NES"],"\np-value = ", df2[p,"pval"],"\nadjusted p-value = ", df2[p,"padj"])
      tmp = plotEnrichment(mfgsea_sets[[p]], ranks,0.5,0.3  ) + 
        labs(title = paste0("Dataset ",t, " | Celltype " ,i," | ", p),
             subtitle = tmp3) + 
        theme(title = element_text(size=5))
      tmp$layers[[5]]$aes_params$colour <- 'firebrick'
      PNG(paste0("LLC.GSEA.",t,".",i,".",p),w=5,h=3)
...
#Heatmap
```{r}
df= LLCDF$DE.LLC.Celltype3.df

selected.cluster = levels(df$Cluster) %>% setdiff("Doublet")
selected.cluster = c("NK","T_CD8","T_CD4","Treg","T_GammaDelta")

geneSet$IL2Signaling = mPathwayList$C2_PID_IL2_STAT5_PATHWAY
geneSet$IL2_IFNG = list("IFNG" = c("Ifng","Ifngr1","Ifngr2"),"IL2" = c("Il2","Il2ra","Il2rb","Il2rg"))

geneSet$tmp = c(geneSet$IFNG,geneSet$IL2R)
geneSet$CEBP = mfgsea_sets_c2$GERY_CEBP_TARGETS
geneSet$ETC = mfgsea_sets$HALLMARK_OXIDATIVE_PHOSPHORYLATION 


for(i in c("Glycolysis","Glycolysis2") ){
  selected.genes = unlist( geneSet[[i]] ) %>% unique() %>% intersect(df$Genes)
  ...
  PNG(paste0("LLC.Heatmap.Identity.Manual.",i), h = 10, w = c(3.5 + (length(unique(df1$Genes))*0.3) ))
  print(plot)
  dev.off()
}
```
